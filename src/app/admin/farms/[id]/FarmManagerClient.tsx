...
export function FarmManagerClient({ farm, initialModels, initialAreaRows, initialDocNumbers,
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* 6: Compartilhar */}
      {activeTab === 6 && (
        <ShareSettingsSection farm={farm} supabase={supabase} showToast={showToast} />
      )}
...
function ShareSettingsSection({ farm, supabase, showToast }: any) {
  const [enabled, setEnabled] = useState<boolean>(farm.share_enabled ?? false);
  const [msg, setMsg] = useState(farm.share_contact_msg ?? "Entre em contato conosco para mais informações sobre esta propriedade.");
  const [link, setLink] = useState(farm.share_contact_link ?? "");
  const [phone, setPhone] = useState(farm.share_contact_phone ?? "");
  const [saving, setSaving] = useState(false);
  const [copied, setCopied] = useState(false);

  const publicUrl = typeof window !== "undefined"
    ? `${window.location.origin}/share/${farm.id}`
    : `/share/${farm.id}`;
  ...
  async function save() {
    ...
    const { error } = await supabase.from("farms").update({
      ...
      share_contact_msg: msg,
      share_contact_link: link,
  ...
  return (
    <div className="space-y-5" style={{ maxWidth: 600 }}>

      {/* Toggle + URL */}
      <div className="atlas-card p-5">
        <h4 className="font-orbitron font-semibold text-xs uppercase tracking-widest mb-4"
          style={{ color: "#00E6FF" }}>Configurações de Compartilhamento</h4>

        {/* Toggle */}
        <div className="flex items-center justify-between mb-5 p-3 rounded"
          style={{ background: "rgba(0,230,255,0.04)", border: "1px solid rgba(0,230,255,0.1)" }}>
          <div>
            <p style={{ fontFamily:"var(--font-main)", fontSize:"0.85rem", fontWeight:600, color:"#F6F8FA" }}>
              Compartilhamento público ativo
            </p>
            <p style={{ fontFamily:"var(--font-main)", fontSize:"0.72rem", color:"#8B949E", marginTop:2 }}>
              Qualquer pessoa com o link consegue acessar a página da fazenda
            </p>
          </div>
          <button
            ...
            style={{
              ...
              background: enabled ? "#00E6FF" : "#2B3138",
          ...
          >
            <span style={{
              ...
              background: enabled ? "#0D1117" : "#8B949E",
        ...
        {enabled && (
          <div className="mb-2">
            <label className="atlas-label">Link público</label>
            <div className="flex gap-2">
              <input
                readOnly
                value={publicUrl}
                className="atlas-input"
                style={{ fontFamily:"monospace", fontSize:"0.8rem", color:"#00E6FF" }}
              />
              <button
                onClick={copyLink}
                className="btn-atlas-outline flex-shrink-0"
                style={{ whiteSpace:"nowrap", fontSize:"0.75rem", padding:"9px 16px" }}
              >
                {copied ? "✓ Copiado" : "Copiar"}
              </button>
              <a
                href={`https://wa.me/?text=${encodeURIComponent("Confira esta propriedade: " + publicUrl)}`}
                target="_blank" rel="noopener noreferrer"
                style={{
                  display:"flex", alignItems:"center", gap:6, padding:"9px 14px", borderRadius:4,
                  border:"1px solid rgba(37,211,102,0.4)", background:"rgba(37,211,102,0.08)",
                  color:"#25d366", fontFamily:"var(--font-main)", fontSize:"0.75rem", fontWeight:600,
      ...
      <div className="atlas-card p-5">
        <h4 className="font-orbitron font-semibold text-xs uppercase tracking-widest mb-4"
          style={{ color: "#00E6FF" }}>CTA de Contato (exibido na página pública)</h4>
        <div className="space-y-4">
          <div>
            <label className="atlas-label">Mensagem de contato</label>
            <textarea
              value={msg}
              onChange={(e) => setMsg(e.target.value)}
              className="atlas-input resize-none"
              rows={3}
              placeholder="Entre em contato conosco para mais informações sobre esta propriedade."
            />
          </div>
          <div>
            <label className="atlas-label">Link de contato (URL)</label>
            <input
              value={link}
              onChange={(e) => setLink(e.target.value)}
              className="atlas-input"
              placeholder="https://wa.me/5511999999999 ou link do site"
            />
          </div>
          <div>
            <label className="atlas-label">Número de telefone (exibição)</label>
            <input
              value={phone}
          ...
          <button onClick={save} className="btn-atlas-primary" disabled={saving}>
