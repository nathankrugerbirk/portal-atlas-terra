...
export function ClientLayout({ children, clientName }: { children: React.ReactNode; clientName?: string }) {
      ...
      {sidebarOpen && (
        ...
        >
          <div className="absolute left-0 top-0 h-full w-60" onClick={(e) => e.stopPropagation()}>
            <ClientSidebar clientName={clientName} onClose={() => setSidebarOpen(false)} />
          </div>
        </div>
      )}
